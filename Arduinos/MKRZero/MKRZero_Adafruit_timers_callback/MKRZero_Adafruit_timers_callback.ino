...
void Timer3Callback0(){
  digitalWrite(TC3_pin, LOW);
}

void Timer3Callback1(){
  digitalWrite(TC3_pin, HIGH);
}
...
void Timer4Callback0(){
  ...
  if (TC4_pin_state){
    digitalWrite(TC4_pin, LOW);
  }
  else {
    digitalWrite(TC4_pin, HIGH);
  }
  TC4_pin_state = !TC4_pin_state;
...
void setup(){
  pinMode(TC3_pin, OUTPUT);
  pinMode(TC4_pin, OUTPUT);
  ...
  zt4.setPeriodMatch(1500, 1000, 0); // 1 match, channel 0
