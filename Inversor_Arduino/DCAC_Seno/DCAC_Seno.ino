...
// Altres valors típics: 38400, 57600, 74880, 115200, 230400, 250000
...
//#include <TimerOne.h>   // No es utilitzable amb MKRZero
#include "Adafruit_ZeroTimer.h" // Hme d'utilitzar esta
...
// Entrades analogiques per variar Freq_PWM i Freq_senoidal
#define FREQ_PWM_PIN A2
...
//#define DIV_SENO 32   // Per Obtindre ones senoidals de 400 Hz DIV_SENO 32. FREQ_PWM = 400 * 32 = 12800
...
// Inicialitzacio Timer1.
  ...
  // Crear LookUp Table Senoidal per 32 divisions
  for (int i = 0; i < 32; i++){
  ...
  // reduir la Inductancia i o Capacitancia del filtre per evitar aquestar deformació.
...
// Adjust de DuttyMax amb Entrada Analogica
    DutyMax=analogRead(FREQ_PWM_PIN);
...
// Adjust de Freq Senoidal amb Entrada Analogica
  ...
  if ( LCD_Updt > 10){ // Actualitzar pantalla cada tants loops
